...
`scripts/package-release.sh` builds the Vite bundle once, cleans and stages `internal/webapp/dist`, then cross-compiles `linux/amd64` and `linux/arm64` with `CGO_ENABLED=0 -tags production -trimpath`. It refuses a working tree with uncommitted tracked changes (unless `RELEASE_ALLOW_DIRTY=1` is set deliberately) so a release artifact always matches the commit it claims. It emits deterministic `tar.gz` archives plus `SHA256SUMS` under the Git-ignored `release/` directory. `scripts/verify-release.sh` checks checksum coverage, the exact archive file set, shipped-file equality with the claimed commit, embedded version/commit strings, AArch64 ELF identity, private- and public-mode amd64 runtime behavior including Superadmin governance and creation-anchored retention, restart persistence, and deterministic rebuilds of both archives.
