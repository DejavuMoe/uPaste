...
Release provenance is enforced before any build: packaging refuses modified tracked files and untracked non-ignored files (both can enter the Go or Vite build while the artifact still claims HEAD), ignores only Git-ignored generated outputs, and rejects a `COMMIT` that is not the checked-out HEAD. `RELEASE_ALLOW_DIRTY=1` exists solely as an explicit local-experiment override and is never set by CI or the release workflow, so no released artifact can claim a commit whose sources were not the ones built.
