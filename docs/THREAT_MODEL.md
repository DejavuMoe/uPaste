...
| Resource ID enumeration | Public network to Share reads | Guessing IDs reveals Standard Shares | Independent 128-bit `crypto/rand` IDs encoded as 22-character unpadded base64url, generic failures, and rate controls; IDs remain identifiers, never authorization secrets. |
...
| Database theft | Filesystem/backup to attacker | Standard content, ciphertext, metadata, and SHA-256 owner-token verifiers are exposed | Least filesystem privilege and protected backups. Uniform 256-bit owner tokens make offline preimage search infeasible without a pepper; document all content and metadata exposure. |
...
| Brute force | Public network to IDs/tokens | Repeated guesses discover Shares or capabilities | 128-bit random IDs, independent 256-bit random owner tokens, constant-time SHA-256 verifier comparison, rate limiting, generic responses, and monitoring without sensitive logs. |
...
| Stale/expired access | Clock/database to read path | Content remains readable after expiration or deletion | Every read derives expiration as `expires_at != NULL && server_now >= expires_at`; deny independently of state transitions or cleanup. Owner deletion removes active payload/row where practical. Test time boundaries. |
