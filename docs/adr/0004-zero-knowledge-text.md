...
For Encrypted text, use browser Web Crypto AES-GCM. Upload ciphertext; place the decryption key in the URL fragment so normal HTTP requests do not transmit it. The server never intentionally receives plaintext or key. Standard mode remains server-visible.
