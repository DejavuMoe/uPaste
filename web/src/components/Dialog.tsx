...
const selector = 'button:not([disabled]):not([hidden]):not([tabindex="-1"]), [href]:not([hidden]):not([tabindex="-1"]), input:not([disabled]):not([hidden]):not([tabindex="-1"]), select:not([disabled]):not([hidden]):not([tabindex="-1"]), textarea:not([disabled]):not([hidden]):not([tabindex="-1"]), [tabindex]:not([tabindex="-1"]):not([disabled]):not([hidden])';

export function Dialog({ title, children, onClose }: { title: string; children: React.ReactNode; onClose: () => void }) {
  const ref = useRef<HTMLDivElement>(null); const onCloseRef = useRef(onClose); const titleId = useId();
